...
const PlaidLinkWithOAuth = () => {
    ...
    React.useEffect(() => {
        const getIsOAuthRedirect = window?.location.href.includes('?oauth_state_id=');
        setIsOAuthRedirect(getIsOAuthRedirect)
    ...
    return isOAuthRedirect ? (
        ...
        <button onClick={() => open()} disabled={!ready} style={{ cursor: 'pointer'}}>
